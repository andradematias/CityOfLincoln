...
module.exports.main = async function (ffCollection, vvClient, response) {
    /**
     * Script Name:     OpPermOperaionalPermitOpenCertificate
     * Customer:        City of Lincoln
     * Purpose:         The purpose of this process is to find and open any
     *                  operational permits certificates that are currently closed
     *                  and associated with a given operational permit.
     *
    ...
    ***********************/
    ...
    try {
        ...
        *****************/
        ...
        // STEP 1 - Find operational permit certificates that are closed with a given operational permit ID.
        let getOperationalPermitCertificatesResponse = await vvClient.forms.getForms({
            q: `[Operational Permit ID] eq '${operationalPermitID}' AND [Status] eq 'Closed'`,
            expand: true
        ...
        // STEP 2 - For each operational permit certificate found, update it's status to pending inspection.
        if (operationalPermitCertificatesLength > 0) {
            for (let operationalPermitCertificate of operationalPermitCertificates) {
                ...
                let openOperationalPermitCertificateResponse = await vvClient.forms.postFormRevision(null, operationalPermitCertificateFields, operationalPermitCertificateTemplateID, operationalPermitCertificateGUID);

                if (openOperationalPermitCertificateResponse.meta.status !== 201) {
                    throw new Error(`An error was encountered when attempting to update the ${operationalPermitCertificateTemplateID} - ${operationalPermitCertificateGUID} form. ${openOperationalPermitCertificateResponse.hasOwnProperty('meta') ? openOperationalPermitCertificateResponse.meta.statusMsg : openOperationalPermitCertificateResponse.message} ${errorMessageGuidance}`);
                }
            }
        }
        ...
        if (operationalPermitCertificates) {
            ...
            outputCollection[1] = 'This permit has been successfully opened.';
