// PrintCertificate for OpPerm Operational Permit
...
var CallServerSide = function () {
    ...
    var requestObject = $.ajax({
        type: "POST",
        url: VV.BaseAppUrl + 'api/v1/' + VV.CustomerAlias + '/' + VV.CustomerDatabaseAlias + '/scripts?name=OpPermOperationalPermitPrintCert',
    ...
    if (typeof (resp.status) != 'undefined') {
        ...
        if (resp.data[0] != undefined) {
            if (resp.data[0] == 'Success') {
                ...
                var popupUrl = VV.BaseURL + "ReportViewer?rpid=75f93588-2788-eb11-81fc-a5e993b5732d&PermitID=" + resp.data[2]
                ...
                messageData = 'An error was encountered. ' + resp.data[1] + errorsReturned;

                VV.Form.HideLoadingPanel();
                VV.Form.Global.DisplayMessaging(messageData);
            }
            else {
                messageData = 'An unhandled response occurred when calling OpPermOperationalPermitPrintCert. The form will not save at this time.  Please try again or communicate this issue to support.';
                VV.Form.HideLoadingPanel();
                VV.Form.Global.DisplayMessaging(messageData);
            }
        }
