...
var CallServerSide = function () {
    ...
    FormInfo.value = VV.Form.Global.datagridFindSelectedRows('DataGrid On Site Contact').join()
    formData.push(FormInfo);

    var FormInfo = {};
    FormInfo.name = 'OperationalPermitID';
    FormInfo.value = VV.Form.GetFieldValue('OperationalPermitID');
    formData.push(FormInfo);

    var FormInfo = {};
    FormInfo.name = 'Designee Type';
    FormInfo.value = 'Type On Site'
    formData.push(FormInfo);
    ...
    if (typeof (resp.status) != 'undefined') {
        ...
        if (resp.data[0] != undefined) {
            if (resp.data[0] == 'Success') {
                var title = 'On Site Assigned'
                var messageData = 'A New On Site Contact has been Assigned.'
