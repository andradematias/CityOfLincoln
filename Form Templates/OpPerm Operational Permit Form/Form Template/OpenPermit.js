...
var CallServerSide = function () {
    ...
    var requestObject = $.ajax({
        type: "POST",
        url: VV.BaseAppUrl + 'api/v1/' + VV.CustomerAlias + '/' + VV.CustomerDatabaseAlias + '/scripts?name=OpPermOperaionalPermitOpenCertificate',
    ...
    if (typeof (resp.status) != 'undefined') {
        ...
        if (resp.data[0] != undefined) {
            if (resp.data[0] == 'Success') {
                VV.Form.SetFieldValue('CloseReasonDD', 'Select Item');
                VV.Form.SetFieldValue('Reason for Closure', '');

                let facilityReopenDate = VV.Form.GetFieldValue('Facility Reopen Date');
                VV.Form.SetFieldValue('Facility Open Date', facilityReopenDate);
                VV.Form.Global.SetFieldUpdate('Status', 'Open');

                VV.Form.DoPostbackSave();
                VV.Form.HideLoadingPanel();
                VV.Form.Global.DisplayMessaging(resp.data[1]);
                ...
                messageData = 'An unhandled response occurred when calling CloseBusiness. The form will not save at this time.  Please try again or communicate this issue to support.';
